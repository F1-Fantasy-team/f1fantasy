...
function SortableDriverRow({ driverId, position, driverName }: { driverId: string; position: number; driverName: string }) {
  const {
    attributes,
    listeners,
    setNodeRef,
  ...
  return (
    <div className="space-y-4">
      <F1Text muted className="block text-xs">
        ...
        {fromApi ? ` (${driverCount} drivers from API)` : ` (${driverCount} drivers — offline list; enable CORS on the API to use the live grid)`}
      </F1Text>
            ...
            <tbody>
              <SortableContext items={driverOrder} strategy={verticalListSortingStrategy}>
                {driverOrder.map((driverId, i) => (
                  <SortableDriverRow key={driverId} driverId={driverId} position={i + 1} driverName={getDriverName(driverId)} />
                ))}
              </SortableContext>
            </tbody>
          </table>
