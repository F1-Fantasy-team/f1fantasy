...
public class ScoringService
{
    ...
    public async Task<int> CalculateMrSaturdayScoreAsync(int groupId, string userId, string season)
    {
        ...
        var allQualifying = racesWithQualifying
            .Where(r => r.QualifyingResults != null)
            .SelectMany(r => r.QualifyingResults!)
            .ToList();
        
        int totalScore = 0;

        // Count P1s (pole positions) for each predicted driver
        if (prediction.Driver1Id != null)
        {
            var poleCount = allQualifying.Count(q => q.Driver?.DriverId == prediction.Driver1Id && q.Position == "1");
            totalScore += poleCount * MR_SATURDAY_QUALI_WIN_POINTS;
        }

        if (prediction.Driver2Id != null)
        {
            var poleCount = allQualifying.Count(q => q.Driver?.DriverId == prediction.Driver2Id && q.Position == "1");
            totalScore += poleCount * MR_SATURDAY_QUALI_WIN_POINTS;
        }
    ...
    private async Task<int> CalculateMrSaturdayScoreForRoundAsync(int groupId, string userId, string season, string round)
    {
        ...
        if (roundRace?.QualifyingResults == null) return 0;

        int points = 0;

        if (prediction.Driver1Id != null)
        {
            var driver1Result = roundRace.QualifyingResults.FirstOrDefault(q => q.Driver?.DriverId == prediction.Driver1Id && q.Position == "1");
            if (driver1Result != null)
            {
        ...
        if (prediction.Driver2Id != null)
        {
            var driver2Result = roundRace.QualifyingResults.FirstOrDefault(q => q.Driver?.DriverId == prediction.Driver2Id && q.Position == "1");
            if (driver2Result != null)
            {
                points += MR_SATURDAY_QUALI_WIN_POINTS;
            }
        }
